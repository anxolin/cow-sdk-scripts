import {
  CowShedHooks,
  CowShedSdk,
  type CoWShedVersion,
} from "@cowprotocol/sdk-cow-shed";
import { AbstractProviderAdapter } from "@cowprotocol/sdk-common";
import { SupportedChainId } from "@cowprotocol/cow-sdk";

// Factory options for the COWShedForComposableCoW deployment on Gnosis Chain.
// See https://gnosisscan.io/address/0x4f4350bf2c74aacd508d598a1ba94ef84378793d#readContract
// and https://github.com/cowdao-grants/cow-shed/pull/53
const COW_SHED_FACTORY_OPTIONS = {
  factoryAddress: "0x4f4350bf2c74aacd508d598a1ba94ef84378793d",
  implementationAddress: "0x6773d5aA31A1EAD34127D564D6E258E66254EbDb", // COWShedForComposableCoW implementation
  proxyCreationCode:
    // See https://gnosisscan.io/address/0x4f4350bf2c74aacd508d598a1ba94ef84378793d#readContract
    "",
} as const;

// EIP-712 domain version of this deployment. The SDK only knows the canonical
// 1.0.0/1.0.1 sheds and defaults to 1.0.1, which this fork rejects with
// `InvalidSignature()`. Only 2.0.0 reproduces the proxy's `domainSeparator()`.
const COW_SHED_DOMAIN_VERSION = "2.0.0" as CoWShedVersion;

/**
 * The base SDK builds its `CowShedHooks` without forwarding the version, nor the
 * factory options on the `signCalls` path. Pin both, so every call site gets the
 * same hooks regardless of which SDK method warms the cache first.
 */
class ComposableCowShedSdk extends CowShedSdk {
  protected getCowShedHooks(chainId: SupportedChainId): CowShedHooks {
    const cached = this.hooksCache.get(chainId);
    if (cached) {
      return cached;
    }

    const hooks = new CowShedHooks(
      chainId,
      { ...COW_SHED_FACTORY_OPTIONS },
      COW_SHED_DOMAIN_VERSION,
    );
    this.hooksCache.set(chainId, hooks);
    return hooks;
  }
}

/**
 * Build a `CowShedSdk` configured for the COWShedForComposableCoW deployment,
 * which is the cow-shed flavour with support for Composable CoW.
 */
export function getCowShedSdk(adapter: AbstractProviderAdapter): CowShedSdk {
  return new ComposableCowShedSdk(adapter, { ...COW_SHED_FACTORY_OPTIONS });
}
